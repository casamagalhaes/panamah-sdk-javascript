...
class BatchProcessor extends EventEmitter {
    ...
    async _sendAccumulatedBatches() {
        ...
        await asyncForEach(batches, async (batch, index, array, stopSendingBatches) => {
            const { status, data: response } = await this._client.post('/stream/data', batch);
            if (status === 200) {
                if (response.falhas) {
    ...
    async _process() {
        try {
            ...
            if (this._running) {
                this._timeoutRunning = setTimeout(() => this._process(), 500);
            } else {
                clearTimeout(this._timeoutRunning);
            }
        } catch (e) {
            this.emit('error', e);
        }
