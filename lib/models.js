...
const generateModelsUsingSchema = () => {
    ...
    const names = keys.map(name => 'Panamah' + name[0].toUpperCase() + camelCase(name.slice(1)));
    return Object.assign({}, ...names.map((name, index) => generateModel(name, keys[index], schema[keys[index]])));
}
