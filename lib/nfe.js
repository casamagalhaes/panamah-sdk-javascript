...
class PanamahNFe {
    ...
    static deserializeCliente (xml) {
        let root = get(xml, 'nfeProc') || xml;
        root = { NFe: get(root, 'NFe[0]') || get(root, 'NFe') };
        if (get(root, 'NFe.infNFe[0].dest[0]'))
    ...
    static deserializeProdutos (xml) {
        ...
        return (dets && dets.map(det => {
            let produto = new PanamahProduto({
                id: get(det, 'prod[0].cProd[0]'),
                descricao: get(det, 'prod[0].xProd[0]'),
                ativo: true
            });
            const eanTributado = get(det, 'prod[0].cEAN[0]') || null;;
            const ean = get(det, 'prod[0].cEAN[0]');
            const validEan = v => ean && !/SEM GTIN/i.test(ean);
            if (validEan(eanTributado) || validEan(ean)) { 
                produto.eans = [];
                if(ean) produto.eans.push({ id: eanTributado, tributado: true });
                if(ean && ean !== eanTributado) produto.eans.push({ id: ean, tributado: false });
            }
            return produto;
        })) || [];
    }
    ...
    static deserializeVenda (xml) {
        ...
        return new PanamahVenda({
            ...
            itens: (dets && dets.map(det => ({
                ...
                preco: Number(get(det, 'prod[0].vProd[0]')) || 0,
                valorUnitario: Number(get(det, 'prod[0].vUnCom[0]')) || 0,
                valorTotal: (Number(get(det, 'prod[0].qCom[0]')) * Number(get(det, 'prod[0].vProd[0]'))) || 0,
