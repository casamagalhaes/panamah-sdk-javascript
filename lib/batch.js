...
const { writeJSON, readJSONSync, forceDirectories, directoryExists, writeJSONSync } = require('./util');
...
class Batch {
    ...
    saveToFileSync(filename) {
        const dirname = path.dirname(filename);
        forceDirectories(dirname);
        return writeJSONSync(filename, this);
    }
    ...
    async saveToDirectorySync(dirname) {
        this.saveToFileSync(path.join(dirname, this.filename));
        return this;
    }
    ...
    remove(id, assinanteId) {
        const targetId = id.id || id;
        const targetAssinanteId = id.assinanteId || assinanteId;
        const index = this._items.findIndex(item => item.id === targetId && (targetAssinanteId ? item.assinanteId === targetAssinanteId : true));
        if (index !== -1)
