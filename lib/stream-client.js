...
const { RefreshTokenError } = require('./exceptions');
...
class StreamClient extends HttpClient {
    ...
    async _makeAuthenticatedRequest(method, url, data, headers) {
        const request = {
            url,
            method,
            headers: Object.assign({}, headers, this._tokens),
            data
        };
    ...
    async _refreshTokens() {
        const { status, data } = await super.get('/stream/auth/refresh', {
            Authorization: this._tokens.refreshToken
        });
        if (status === 200) return data;
        else throw new RefreshTokenError('Erro ao renovar o token do stream.');
    }

    async _calculateKey(secret, assinanteId, ts) {
        return sha1Base64(secret + assinanteId + ts);
    }

    async _authenticate() {
        const ts = moment.utc().unix();
        const { status, data } = await super.post(
            '/stream/auth',
            {
        ...
        if (status === 200) return data;
        else throw new AuthError('Credenciais inválidas ou serviço indisponível.');
    }
}
